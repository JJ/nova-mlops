...
* Achieving collective authorship of the code; this dilutes responsibility, but at the same time the whole time acquires responsibility on the code.
* Knowledge transfer between different members of the team: Some people might work in some areas that are more in their turf, and some in others, but eventually the code is produced by the team, not by any individual.
* Effectively checking the code against team standards (such as identifier names), oversights, or simply offer alternatives to the technical solutions proposed by one member of the team.

Remember, pull requests reviews are not grades in an exam, they are not obstacles for *your* code to be merged, they are simply a way to ensure quality of the result, and make it a collective endeavor, as opposed to a single person.
